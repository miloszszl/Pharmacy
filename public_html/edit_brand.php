...
{
    $select_brands_q=mysqli_query($link,"SELECT * FROM Marki ORDER BY nazwa;");
    if($select_brands_q)
    {
        $brands_data="";
        
        if($select_brands_q and mysqli_num_rows($select_brands_q)>0)
        {
            while($row=mysqli_fetch_assoc($select_brands_q))
            {  
                $brands_data.="<form class=\"tr white_a\" method=\"GET\" action=\"brand_edit_panel.php\"><span class=\"td\">{$row['nazwa']}</span><span class=\"td\"><button type=\"submit\" name=\"brand_id\" value=\"{$row['idMarki']}\" class=\"btn btn-success btn-lg\">Edytuj</button></span><span class=\"td\"><button onclick=\"delete_brand(this,this.parentElement.parentElement)\" type=\"button\" name=\"delete_button\" value=\"{$row['idMarki']}\" class=\"btn btn-success btn-lg\">Usuń</button></span></form>";
            }
        }
    }   
}
...
<div class="container"> 
    ...
    <div id="admin_answer" style="text-align:center;"></div>
            <div id="admin_panel" style="color:white;">
                <h2 style="text-align:center;">Edycja i usuwanie marek</h2>
                    ...
                    <div class="table center" >
                        ...
                        <?php
                            if(isset($brands_data)) echo $brands_data;
                        ?>
                    </div>
                    </div>
                <br><br>
...
<script>
    ...
    function delete_brand(td,form)
    {
         var x=confirm("Czy napewno chcesz usunąć tą markę?");
        ...
        try
        {
            ...
            if (this.readyState == 4 && this.status == 200) {
                ...
                {
                    answerBox.html("<h2 style=\"color:#fc7676;\">Nie można usunąć marki</h2>");
                }
                ...
                {   
                    form.remove()
                    answerBox.html("<h2 style=\"color:#8af294;\">Pomyślnie usunięto markę</h2>");
                }
            }
            };
            xhttp.open("POST", "delete_brand.php", true);
